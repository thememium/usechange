...
def test_parse_commit_conventional() -> None:
    commit = GitCommit(sha="abc1234", subject="feat(core): add feature", body="", author_name="Dev", author_email="dev@test.com")
...
def test_parse_commit_breaking_bang() -> None:
    commit = GitCommit(sha="abc1234", subject="feat!: breaking change", body="", author_name="Dev", author_email="dev@test.com")
    parsed = _parse_commit(commit)
    assert parsed.breaking is True


def test_parse_commit_breaking_footer() -> None:
    commit = GitCommit(sha="abc1234", subject="feat: add", body="BREAKING CHANGE: removed", author_name="Dev", author_email="dev@test.com")
    parsed = _parse_commit(commit)
    assert parsed.breaking is True


def test_parse_commit_breaking_change_with_dash() -> None:
    commit = GitCommit(sha="abc1234", subject="feat: add", body="BREAKING-CHANGE: something", author_name="Dev", author_email="dev@test.com")
    parsed = _parse_commit(commit)
    assert parsed.breaking is True


def test_parse_commit_references() -> None:
    commit = GitCommit(sha="abc1234", subject="fix: bug #42", body="Also #99", author_name="Dev", author_email="dev@test.com")
    parsed = _parse_commit(commit)
...
def test_parse_commit_no_scope() -> None:
    commit = GitCommit(sha="abc1234", subject="feat: add feature", body="", author_name="Dev", author_email="dev@test.com")
    parsed = _parse_commit(commit)
...
def test_group_commits_basic() -> None:
    types = default_types()
    commits = [
        ParsedCommit(sha="a", short_sha="a", type="feat", scope=None, subject="new thing", breaking=False, author_name="Dev", author_email="dev@test.com", references=[]),
        ParsedCommit(sha="b", short_sha="b", type="fix", scope=None, subject="bug fix", breaking=False, author_name="Dev", author_email="dev@test.com", references=[]),
        ParsedCommit(sha="c", short_sha="c", type="other", scope=None, subject="misc", breaking=False, author_name="Dev", author_email="dev@test.com", references=[]),
    ]
...
def test_group_commits_with_scope_and_refs() -> None:
    types = default_types()
    commits = [
        ParsedCommit(sha="a", short_sha="a123456", type="feat", scope="core", subject="feature", breaking=False, author_name="Dev", author_email="dev@test.com", references=["#42"]),
    ]
...
def test_group_commits_without_repo_info() -> None:
    types = default_types()
    commits = [
        ParsedCommit(sha="abc1234567", short_sha="abc1234", type="fix", scope=None, subject="bug", breaking=False, author_name="Dev", author_email="dev@test.com", references=[]),
    ]
    sections = _group_commits(commits, types, None)
    item = sections[0].items[0]
...
def test_collect_contributors_basic() -> None:
    commits = [
        ParsedCommit(sha="a", short_sha="a", type="feat", scope=None, subject="s", breaking=False, author_name="Alice", author_email="alice@test.com", references=[]),
        ParsedCommit(sha="b", short_sha="b", type="fix", scope=None, subject="s", breaking=False, author_name="Bob", author_email="bob@test.com", references=[]),
    ]
...
def test_collect_contributors_dedup() -> None:
    commits = [
        ParsedCommit(sha="a", short_sha="a", type="feat", scope=None, subject="s", breaking=False, author_name="Alice", author_email="alice@test.com", references=[]),
        ParsedCommit(sha="b", short_sha="b", type="fix", scope=None, subject="s", breaking=False, author_name="Alice", author_email="alice@test.com", references=[]),
    ]
    contributors = _collect_contributors(commits, [], False)
...
def test_collect_contributors_exclude() -> None:
    commits = [
        ParsedCommit(sha="a", short_sha="a", type="feat", scope=None, subject="s", breaking=False, author_name="Bot", author_email="bot@test.com", references=[]),
    ]
...
def test_merge_changelog_creates_new(tmp_path: Path) -> None:
    from usechange.changelog.markdown import ReleaseNotes, ChangeSection
    notes = ReleaseNotes(version="v1.0.0", date=None, sections=[ChangeSection(title="Features", items=["feat A"])])
...
def test_merge_changelog_merges_existing(tmp_path: Path) -> None:
    ...
    notes = ReleaseNotes(version="v1.0.0", date=None, sections=[ChangeSection(title="Features", items=["new"])])
...
def test_merge_changelog_replaces_same_version(tmp_path: Path) -> None:
    ...
    existing = "# Changelog\n\n## v1.0.0\n\n### Old\n\n- old item\n\n"
    (tmp_path / "CHANGELOG.md").write_text(existing)
    notes = ReleaseNotes(version="v1.0.0", date=None, sections=[ChangeSection(title="New", items=["new item"])])
    content = _merge_changelog(str(tmp_path), "CHANGELOG.md", notes)
...
def test_format_commit_with_scope_and_url() -> None:
    commit = ParsedCommit(sha="abc1234567", short_sha="abc1234", type="feat", scope="core", subject="feature", breaking=False, author_name="D", author_email="d@t.com", references=["#1"])
...
def test_format_commit_no_scope_no_url() -> None:
    commit = ParsedCommit(sha="abc1234567", short_sha="abc1234", type="fix", scope=None, subject="bug", breaking=False, author_name="D", author_email="d@t.com", references=[])
...
def test_run_changelog_basic(tmp_path: Path) -> None:
    ...
    options = _default_options(directory=str(tmp_path), no_output=True, include_date=False)
...
def test_run_changelog_with_output(tmp_path: Path) -> None:
    _init_repo(tmp_path)
    _make_commit(tmp_path, "feat: add feature")
    options = _default_options(directory=str(tmp_path), no_output=False, output="CHANGELOG.md")
    result = run_changelog(options)
...
def test_run_changelog_hide_author_email(tmp_path: Path) -> None:
    _init_repo(tmp_path)
    _make_commit(tmp_path, "feat: initial")
    options = _default_options(directory=str(tmp_path), hide_author_email=True, no_output=True)
    result = run_changelog(options)
    assert "<" not in result.content.split("Contributors")[-1] if "Contributors" in result.content else True


def test_run_changelog_release_version(tmp_path: Path) -> None:
    _init_repo(tmp_path)
    _make_commit(tmp_path, "feat: initial")
    options = _default_options(directory=str(tmp_path), release_version="5.0.0", no_output=True)
    result = run_changelog(options)
...
def test_run_changelog_with_date(tmp_path: Path) -> None:
    _init_repo(tmp_path)
    _make_commit(tmp_path, "feat: initial")
    options = _default_options(directory=str(tmp_path), include_date=True, no_output=True)
    result = run_changelog(options)
...
def test_run_changelog_no_emojis(tmp_path: Path) -> None:
    _init_repo(tmp_path)
    _make_commit(tmp_path, "feat: initial")
    options = _default_options(directory=str(tmp_path), include_emojis=False, no_output=True)
    result = run_changelog(options)
...
def test_run_changelog_from_ref(tmp_path: Path) -> None:
    ...
    options = _default_options(directory=str(tmp_path), from_ref="v1.0.0", no_output=True)
    result = run_changelog(options)
...
def test_run_changelog_update_versions(tmp_path: Path) -> None:
    ...
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text('name = "test"\nversion = "1.0.0"\n')
    options = _default_options(directory=str(tmp_path), bump=True, no_output=True, update_versions=True)
    result = run_changelog(options)
...
def test_run_changelog_premajor_bump(tmp_path: Path) -> None:
    ...
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text('name = "test"\nversion = "1.0.0"\n')
    options = _default_options(directory=str(tmp_path), premajor="alpha", no_output=True)
    result = run_changelog(options)
...
def test_run_changelog_prerelease_bump(tmp_path: Path) -> None:
    ...
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text('name = "test"\nversion = "1.0.0"\n')
    options = _default_options(directory=str(tmp_path), prerelease="dev", no_output=True)
    result = run_changelog(options)
...
def test_run_changelog_preview_next_version(tmp_path: Path) -> None:
    ...
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text('name = "test"\nversion = "1.0.0"\n')
    options = _default_options(directory=str(tmp_path), preview_next_version=True, no_output=True)
    result = run_changelog(options)
...
def test_run_changelog_canary(tmp_path: Path) -> None:
    ...
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text('name = "test"\nversion = "1.0.0"\n')
    options = _default_options(directory=str(tmp_path), canary="true", no_output=True, bump=True)
    result = run_changelog(options)
    assert result.new_version is not None
...
def test_run_changelog_version_suffix(tmp_path: Path) -> None:
    ...
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text('name = "test"\nversion = "1.0.0"\n')
    options = _default_options(directory=str(tmp_path), version_suffix="beta.1", no_output=True, bump=True)
    result = run_changelog(options)
